...
//处理金额，除以100
template.helper('initMoney',function (money) {
    if(money){
...
//处理时间   2019-5-15 15:33
template.helper('initTime',function (time) {
    if(time){
        return quakooUtils.formatTimeToDate(time)
    }else{
        return 0
    }
})
//处理时间   刚刚/1分前，一小时前
template.helper('initTimeToNow',function (time) {
    if(time){
        return quakooUtils.getDateDiff(time)
    }else{
        return 0
    }
})
